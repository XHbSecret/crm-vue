...
<template>
  <header id="header">
    <el-row type="flex" justify="space-between">
      ...
      >
      <el-col :span="5">
        <el-input placeholder="请输入产品名" v-model="productQuery.productName">
          <template #append>
  ...
  <div v-show="productList.datas.length > 0" id="head">
    ...
    <el-button type="" @click="yichushou()">已出租</el-button>
  </div>
  <div v-show="productList.datas.length == 0" id="head">
    <span>场景：</span>
    <el-select
  ...
  >
    ...
    <el-table-column prop="productSellable" label="是否已售卖/出租" width="180">
    ...
    <el-table-column prop="productStatus" label="产品状态" width="180">
      <template v-slot="scope">
        <el-tag type="success" v-if="scope.row.productStatus == 1">上架</el-tag>
    ...
    <el-table-column prop="productDecorateType" label="装修类型" width="180" />
  </el-table>
  <el-pagination
