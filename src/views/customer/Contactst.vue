...
<template>
  <div class="Contactst">
    <div>
      <el-button @click="addlxr" type="warning">添加联系人</el-button>
    </div>
    ...
    <el-table :data="data.custList" style="width: 100%">
      ...
      >
        <template v-slot:default="scope">
          <el-tag
            class="ml-2"
            type="success"
            v-if="scope.row.contactIsPolicy == 0"
            >是</el-tag
          >
          <el-tag
            class="ml-2"
            type="danger"
            v-if="scope.row.contactIsPolicy == 1"
            >否</el-tag
          >
        </template>
      </el-table-column>
      ...
      <el-table-column fixed="right" label="操作" width="180">
        <template #default="{ row }">
          <!-- handleEdit触发事件：修改此表 -->
          <el-button type="text" size="small" @click="updateContactst(row)" v-if="row.contactIsPolicy!=0"
            >设为首要联系人</el-button
          >
          <span  style="font-size:12px;margin-right: 20px;" v-else
            >首要联系人</span
          >
          <el-button type="text" size="small" @click="deleteContactst(row)" v-if="row.contactIsPolicy!=0"
            >删除</el-button
          >
        </template>
      </el-table-column>
...
const updateContactst = (val) => {
  api.customer.updateCustomer(val).then((response) => {
